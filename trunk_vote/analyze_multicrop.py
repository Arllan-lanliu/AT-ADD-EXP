...
from trunk_vote.crop_dataset import MultiCropEvalDataset
...
def main():
    ...
    dev_set = MultiCropEvalDataset(path_to_audio=args.eval_audio, audio_length=args.audio_len)
    ...
    logits_by_name = defaultdict(list)
    feats_by_name = defaultdict(list)
    with open(score_path, "w", newline="", encoding="utf-8") as sf:
        ...
        with torch.no_grad():
            for wave, filenames in tqdm(dev_loader, desc="dev-analysis"):
                ...
                for i, fn in enumerate(filenames):
                    ...
                    if meta is None:
                        continue
                    t, y = meta
                    logits_by_name[name].append(logits_np[i])
                    feats_by_name[name].append(feat_np[i].reshape(-1))
            ...
            for name in sorted(logits_by_name):
                meta = label_meta.get(name)
                if meta is None:
                    continue
                t, y = meta
                mean_logits = np.stack(logits_by_name[name], axis=0).mean(axis=0)
                probs_real = torch.softmax(
                    torch.tensor(mean_logits, dtype=torch.float32).view(1, -1),
                    dim=1,
                )[0, 0].item()
                predict = "real" if probs_real >= 0.5 else "fake"
                writer.writerow([
                    name,
                    float(probs_real),
                    float(mean_logits[0]),
                ...
                if not args.metrics_only and feats_by_name.get(name):
                    tsne_features.append(np.stack(feats_by_name[name], axis=0).mean(axis=0))
