...
def gen_score(model: torch.nn.Module, args) -> None:
    """Run crop inference, mean logits per audio, and write scores."""
    ...
    with torch.no_grad():
        for waveform, filenames in tqdm(loader, desc="scoring"):
            ...
            for fn, logits in zip(filenames, outputs):
                logits_by_name.setdefault(fn.strip(), []).append(logits)

    with open(args.score_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["name", "score"])
        for name in sorted(logits_by_name):
            logits = torch.stack(logits_by_name[name], dim=0).mean(dim=0, keepdim=True)
