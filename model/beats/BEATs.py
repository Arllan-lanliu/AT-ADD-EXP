...
class BEATs(nn.Module):
    ...
    def extract_features(
        ...
        encoder_tgt_layer = None
        if tgt_layer is not None:
            # Public hidden-state indices use 0=input, 1..N=encoder layer outputs.
            # TransformerEncoder.tgt_layer is zero-based over encoder layers.
            encoder_tgt_layer = max(int(tgt_layer) - 1, 0)
        elif return_hidden_states:
            encoder_tgt_layer = int(self.cfg.encoder_layers) - 1

        x, layer_results = self.encoder(
            x,
            padding_mask=padding_mask,
            layer=encoder_tgt_layer,
        )
