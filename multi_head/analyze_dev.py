...
def run_analyze_dev(ns: argparse.Namespace) -> None:
    ...
    for strategy in strategies:
        scores, labels, types, names = _collect_scores_one_strategy(
            model,
            loader,
            strategy=strategy,
            device=args.device,
        ...
        with open(csv_path, "w", newline="", encoding="utf-8") as fh:
            w = csv.writer(fh)
            w.writerow(["name", "score", "type", "label", "predict"])
            for i in range(len(names)):
                tn = IDX_TO_TYPE[int(types[i])]
                ylbl = "real" if labels[i] == 0 else "fake"
                w.writerow([names[i], f"{scores[i]:.8f}", tn, ylbl, preds_str[i]])
        ...
        print(classification_report(labels, preds_lbl, target_names=["real", "fake"], digits=4))
